...
public class WordClock.BootSequenceRenderer : GLib.Object, Jsonable, ClockRenderable, MatrixRenderer, DotsRenderer, BacklightRenderer {
	...
	public bool render_matrix( Color[,] leds_matrix ) {
		if(this.frame <= 25) {
			...
			for(int i=0; i<leds_matrix.length[0]; i++) {
				for(int j=0; j<leds_matrix.length[1]; j++) {
					leds_matrix[i,j].set_hsv( 0, 0, (uint8) (1000-this.frame*10) );
				}
			}
			
			return true;
		}else if(this.frame == 101) {
	...
	public bool render_dots( Color[] leds_dots ) {
		if(this.frame <= 25) {
			...
			for(int i=0; i<leds_dots.length; i++) {
				leds_dots[i].set_hsv( 0, 0, (uint8) (1000-this.frame*10) );
			}
		}
	...
	public bool render_backlight( Color[] leds_backlight ) {
		if(this.frame > 100 && this.frame <= 125) {
			for(int i=0;i<leds_backlight.length;i++) {
				leds_backlight[i].set_hsv( 0, 0, (uint8) (this.frame*10-1000) );
			}
		}else if(this.frame > 125) {
			for(int i=0;i<leds_backlight.length;i++) {
				leds_backlight[i].set_hsv( 0, 0, 255 );
			}
			leds_backlight[(leds_backlight.length+this.frame-124)%leds_backlight.length].set_hsv( (uint16) ((this.frame*3) % 360), 255, 255 );
			leds_backlight[(leds_backlight.length+this.frame-125)%leds_backlight.length].set_hsv( (uint16) ((this.frame*3) % 360), 255, 255 );
			leds_backlight[(leds_backlight.length+this.frame-126)%leds_backlight.length].set_hsv( (uint16) ((this.frame*3) % 360), 255, 255 );
			leds_backlight[(leds_backlight.length+this.frame-127)%leds_backlight.length].set_hsv( (uint16) ((this.frame*3) % 360), 255, 255 );
			leds_backlight[(leds_backlight.length+this.frame-128)%leds_backlight.length].set_hsv( (uint16) ((this.frame*3) % 360), 255, 255 );
		}
